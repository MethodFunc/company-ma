...
def ttest_2samp(df1, df2):
    ...
    print(f'{p_value_05.keys()}의 p_value의 값이 0.05이하임으 두 집단간의 평균의 차이가 있다.')
...
def calc_mann(df, df2):
    ''' 2 dataframe 비모수 검정'''
    for col in df.columns:
        s, p = stats.mannwhitneyu(df[col], df2[col])
        if p < 0.05:
            print(f"{col}의 p_value값은 {p:.4f}으로 두 집단간의 평균의 차이가 있다.")
...
def shapiro_test(df, alpha=0.05):
    ...
    print(f"{col}의 값은 0.05이상이므로 정규성을 만족한다.")
